class HrmdashboardController < ApplicationController
  ...
  def team
    ...
    if (params[:search_term].present?) && (params[:department_id].present?)
      @team = @team.where("ttid LIKE ? or LOWER(first_name) LIKE ? or LOWER(middle_name) LIKE ? or LOWER(last_name) LIKE ? or email LIKE ? or contact_no LIKE ? and department_id =?", "%#{params[:search_term]}%", "%#{params[:search_term]}%", "%#{params[:search_term]}%", "%#{params[:search_term]}%", "%#{params[:search_term]}%", "%#{params[:search_term]}%", params[:department_id])
    elsif params[:search_term].present?
      @team = @team.where("ttid LIKE ? or LOWER(first_name) LIKE ? or LOWER(middle_name) LIKE ? or LOWER(last_name) LIKE ? or email LIKE ? or contact_no LIKE ?", "%#{params[:search_term]}%", "%#{params[:search_term]}%", "%#{params[:search_term]}%", "%#{params[:search_term]}%", "%#{params[:search_term]}%", "%#{params[:search_term]}%")
